...
横幅が修正された！
書き方をエモーションっていうらしい
ディレクトリーに分割して、tsx index.tsxってするとtsxを

# 2024/7/15
## 進捗
- backendのディレクトリ構成をMVCモデルに沿って分割した。
  - GoとMySQLでローカルで動く簡単なアプリを作成した。MVCモデルに沿ってディレクトリを分けたりログを出力させたりしたが、このアプリのおかげで前に分割できずに断念したものが分割できるようにできた。自分を褒めます
- フロントエンドのログについて
  - Dockerで出力させてもいいが、別にいらないと考え直した。
  - 特に大量のログが生成される場合、コンテナの応答性が低下する
  - ブラウザの開発者ツールで確認できるため、Dockerコンテナに出力する必要はない
  - Dockerコンテナ内でログを出力しても実際のユーザーが直面する問題を完全に再現できるとは限らない
